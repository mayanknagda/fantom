...
from ._ecrtm import get_ecr_cost, ECR
...
class VAE(BaseModel):
    ...
    def __init__(self, encoder, sampler, decoder, nll_loss, kld_loss, **kwargs) -> None:
        ...
        if ("ecrtm" in self.model_name) and ("etm" in self.model_name):
            self.ecr = ECR()
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Encode the input
        if "context" in self.model_name:
            doc_emb = kwargs["doc_emb"]
            x_in = torch.cat([x, doc_emb], dim=1)
        ...
        if ("ecrtm" in self.model_name) and ("etm" in self.model_name):
            ecr_loss = self.ecr(
                get_ecr_cost(
                    self.decoder.topic_embeddings.weight.data.T,
                    self.decoder.word_embeddings.weight.data,
                )
            )
            nll_loss += ecr_loss
